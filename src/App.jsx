...
function buildGroupMatches() {
  ...
  for (const [grp, teams] of Object.entries(GROUPS)) {
    pairs.forEach(([i,j]) => {
      rows.push({ id:`groups-${grp}-${idx++}`, phase:'groups', grp, t1:teams[i], t2:teams[j], s1:'', s2:'', pen1:'', pen2:'' })
    })
  }
  return rows
}
...
function buildKnockoutMatches() {
  ...
  for (const phase of PHASE_ORDER) {
    for (let i = 0; i < PHASE_COUNT[phase]; i++) {
      rows.push({ id:`${phase}-${idx++}`, phase, grp:null, t1:'', t2:'', s1:'', s2:'', pen1:'', pen2:'' })
    }
  }
  return rows
}
...
function MatchRow({ m, onScore, locked, isAdmin }) {
  ...
  return (
    <div style={{background:'#0d1b2a',borderRadius:10,padding:'10px 12px',
      border:'1px solid #1e3a5f',marginBottom:6,opacity:m.t1||m.phase==='groups'?1:0.4}}>
...
export default function App() {
  ...
  async function loadMyQuiniela() {
    ...
    if (data) {
      const q = {}
      data.forEach(r => { q[r.match_id] = { s1: r.s1, s2: r.s2 } })
      setQuiniela(q)
    }
  }
  ...
  async function fetchRealScores() {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        method:'POST', headers:{method:'POST', headers:{'Content-Type':'application/json','x-api-key':import.meta.env.VITE_ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},},
        body: JSON.stringify({
  ...
  return (
    <div style={{fontFamily:'system-ui,sans-serif',background:'#0a0e1a',minHeight:'100vh',color:'#e8eaf6'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#1a237e,#283593,#1565c0)',padding:'14px 16px 0',textAlign:'center'}}>
        ...
        <div style={{maxWidth:900,margin:'0 auto 6px',display:'flex',justifyContent:'center',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          {isAdmin && <button onClick={fetchRealScores} disabled={aiLoading}
            style={{background:aiLoading?'#37474f':'#f57f17',border:'none',borderRadius:20,
              padding:'6px 16px',color:'#fff',fontWeight:700,cursor:aiLoading?'not-allowed':'pointer',fontSize:12}}>
      ...
      <div style={{padding:16,maxWidth:900,margin:'0 auto'}}>
        ...
        {/* SELECTOR GRUPO */}
        {['grupos','partidos','quiniela'].includes(tab) && (
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
              ...
              <tbody>
                {groupStandings[activeGroup].map(([team,s],i)=>(
                  <tr key={team} style={{borderTop:'1px solid #1e3a5f',
                    background:i<2?'rgba(21,101,192,0.18)':i===2?'rgba(255,183,0,0.07)':'transparent'}}>
        ...
        {tab==='eliminatorias' && PHASE_ORDER.map(phase=>{
          ...
          return (
            <div key={phase} style={{marginBottom:16}}>
              <div style={{fontWeight:800,fontSize:13,color:PHASE_COLORS[phase],
                borderLeft:`3px solid ${PHASE_COLORS[phase]}`,paddingLeft:10,marginBottom:8}}>
                {PHASE_LABELS[phase]}
              </div>
              {ms.map(m=>(
                <div key={m.id} style={{background:'#0d1b2a',borderRadius:10,padding:'10px 12px',
                  border:'1px solid #1e3a5f',marginBottom:6,opacity:m.t1?1:0.4}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                    <div style={{flex:1,textAlign:'right'}}>
                      {isAdmin&&!isLocked
                        ? <input value={m.t1} onChange={e=>updateKnockoutTeam(m.id,'t1',e.target.value)}
                            placeholder="Equipo 1"
                            style={{background:'#1e2d3d',border:'1px solid #37474f',borderRadius:6,
                              color:'#fff',padding:'4px 8px',fontSize:12,width:120,textAlign:'right'}}/>
                    ...
                    <div style={{flex:1,textAlign:'left'}}>
                      {isAdmin&&!isLocked
                        ? <input value={m.t2} onChange={e=>updateKnockoutTeam(m.id,'t2',e.target.value)}
                            placeholder="Equipo 2"
                            style={{background:'#1e2d3d',border:'1px solid #37474f',borderRadius:6,
                              color:'#fff',padding:'4px 8px',fontSize:12,width:120}}/>
        ...
        {tab==='bracket' && (
          <div style={{overflowX:'auto',paddingBottom:12}}>
            <div style={{display:'flex',gap:10,minWidth:1100,alignItems:'flex-start'}}>
              {PHASE_ORDER.filter(p=>p!=='tp').map(phase=>(
                <div key={phase} style={{minWidth:165,flex:'0 0 auto'}}>
                  <div style={{textAlign:'center',fontWeight:700,fontSize:11,color:PHASE_COLORS[phase],
                    background:'rgba(0,0,0,0.3)',borderRadius:8,padding:'4px 8px',marginBottom:8}}>
                    {PHASE_LABELS[phase]}
                  </div>
                  {matches.filter(m=>m.phase===phase).map(m=>(
                    <div key={m.id} style={{background:'#0d1b2a',border:'1px solid #1e3a5f',
                      borderRadius:8,padding:'8px 10px',marginBottom:8,fontSize:12}}>
            ...
            {(()=>{
              ...
              return(
                <div style={{textAlign:'center',marginTop:20,background:'linear-gradient(135deg,#f57f17,#f9a825)',
        ...
        {tab==='ranking' && (
          <div>
            <div style={{background:'#0d1b2a',borderRadius:12,overflow:'hidden',border:'1px solid #1e3a5f'}}>
              ...
              {ranking.length===0&&(
                <div style={{padding:20,textAlign:'center',color:'#546e7a',fontSize:13}}>
                  Aún no hay participantes con pronósticos guardados.
                </div>
              )}
              {ranking.map(({nick,pts},i)=>(
                <div key={nick} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 16px',
                  borderTop:'1px solid #1e3a5f',
                  background:nick===nickname?'rgba(21,101,192,0.2)':'transparent'}}>
                  <span style={{fontWeight:800,fontSize:18,width:28,textAlign:'center',
                    color:i===0?'#ffd600':i===1?'#b0bec5':i===2?'#ff8a65':'#546e7a'}}>
                    {i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}
                  </span>
                  <span style={{flex:1,fontWeight:nick===nickname?700:400}}>
                    {nick} {nick===nickname&&'(tú)'}
                  </span>
